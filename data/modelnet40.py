    ...
    def __init__(
        ...
        **kwargs,
        ...
        (
            ...
            self.test_transform,
        ) = self.get_modelnet40(root, train_transform, test_transform, target_transform)

        if "num_classes" in kwargs:
            print(
                f"Warning: num_classes is not used for {self.__class__.__name__} dataset. Ignoring the argument and using default number of classes."
            )
    ...
    def get_modelnet40(
        ...
        if not os.path.exists(downloaded_file):
            ...
            downloaded_file.write_bytes(
                requests.get(self.MODELNET40_URL, verify=False).content
            )
        else:
