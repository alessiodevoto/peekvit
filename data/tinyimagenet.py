...
class TinyImageNet:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        (
            ...
            self.test_transform,
        ) = self.get_tiny_imagenet(
            root, train_transform, test_transform, target_transform
        )

        if "num_classes" in kwargs:
            print(
                f"Warning: num_classes is not used for {self.__class__.__name__} dataset. \nIgnoring the argument and using default number of classes in this dataset (200)."
            )
    ...
    def get_tiny_imagenet(
        self, root, train_transform=None, test_transform=None, target_transform=None
    ):
        """
        ...
        train_path, val_path = [
            pathlib.Path(root) / folder
            for folder in ["tiny-imagenet-200/train/", "tiny-imagenet-200/val/"]
        ]
        ...
        train_dataset = torchvision.datasets.ImageFolder(
            train_path, train_transform, target_transform
        )
        val_dataset = torchvision.datasets.ImageFolder(
            val_path, test_transform, target_transform
        )
