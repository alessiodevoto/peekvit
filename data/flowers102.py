...
class Flowers102Dataset:
    ...
    FLOWERS202_DENORMALIZE_TRANSFORM =  T.Compose([
    ...
    def __init__(self,
        ...
        self.denormalize_transform = self.FLOWERS202_DENORMALIZE_TRANSFORM

        if 'num_classes' in kwargs:
            print(f'Warning: num_classes is not used for {self.__class__.__name__}. \nIgnoring the argument and using default number of classes in this dataset (102).')
        
        self.train_dataset, self.val_dataset = None, None
        if 'train' in split:
            self.train_dataset = Flowers102(root, split='train', download=download, transform=train_transform, target_transform=target_transform)
        if 'test' in split:
            self.val_dataset = Flowers102(root, split='test', download=download, transform=test_transform, target_transform=target_transform)
