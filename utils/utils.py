...
def get_moes(model):
    ...
    for module_name, module in model.named_modules():
        if (
            isinstance(module, MoE) and module.num_experts > 1
        ):  # only add MoE modules with more than 1 expert
...
def save_state(
    path, model, model_args, noise_args, optimizer, epoch, skip_optimizer=True
):
    """
...
def load_state(path, model: Any = None, optimizer: Any = None, strict: bool = False):
    ...
    try:
        ...
        if len(res[0]) > 0:
            print(
                "Some parameters are not present in the checkpoint and will be randomly initialized: ",
                res[0],
            )
    except RuntimeError as e:
        print(e)
        print(
            "The model state dict could not be loaded. This is probably because the checkpoint has a different architecture."
        )
