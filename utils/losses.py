...
def sparsity_loss_per_block(
    model,
    budget: float = 0.65,
    sparsity_type: Literal["l1", "mse", "cross_entropy"] = "l1",
    **kwargs,
):
    """
    ...
    for _, mask in masks.items():
        ...
        sparsity = reduce(
            mask, "b s 1 -> b", "mean"
        )  # this is basically the percentage of 1s in the mask
        ...
        if sparsity_type == "cross_entropy":
            ...
            sparsity_loss.append(
                cross_entropy(
                    sparsity,
                    torch.tensor([budget] * sparsity.shape[0]).to(
...
class ChannelMSELoss(ResidualModelLoss):
    ...
    def forward(self, model, channel_budget=None, **kwargs):
        ...
        """
        assert (
            channel_budget or self.budget
        ), "budget must be provided either as argument or as class attribute"

        return solo_mse(
            model,
            channel_budget if channel_budget is not None else self.budget,
            self.strict,
...
class LossCompose:
    ...
    def __init__(self, losses_dict):
        ...
        for loss, loss_args in losses_dict.items():
            self.additional_losses[loss]["weight"] = loss_args.pop("weight", 1.0)
