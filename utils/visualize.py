...
def string_to_color(s):
    ...
    color = plt.cm.tab10(
        hash_value % 10
    )  # Use viridis colormap, but you can choose any colormap
    return color
...
def plot_timing_recap(
    timings_per_budgets, timings_per_flops, save_dir, additional_label=""
):
    if timings_per_budgets is not None:
        ...
        plt.ticklabel_format(style="sci", axis="x", scilimits=(0, 0))
        plt.savefig(
            os.path.join(save_dir, f"budget_vs_throughput{additional_label}.png")
        )
    ...
    if timings_per_flops is not None:
        ...
        plt.ticklabel_format(style="sci", axis="x", scilimits=(0, 0))
        plt.savefig(
            os.path.join(save_dir, f"flops_vs_throughput{additional_label}.png")
        )
...
def plot_cumulative_budget_recap(
    ...
    if run_accs_per_flops is not None:
        ...
        for i, (run_id, accs_per_flops) in enumerate(run_accs_per_flops.items()):
            ax.plot(
                accs_per_flops.keys(),
                accs_per_flops.values(),
                marker="o",
                color=string_to_color(i),
            )
            ax.set_xlabel("Flops")
            ax.set_ylabel("Accuracy")
            ax.set_title("Flops vs Accuracy")
        ...
        plt.legend(run_names or [x.split("/")[-1] for x in run_accs_per_flops.keys()])
        plt.savefig(
            os.path.join(save_dir, f"cumulative_flops_vs_acc{additional_label}.png")
        )


def plot_budget_and_noise_recap(
    accs_per_budget, accs_per_flops, save_dir, additional_label=""
):
    if accs_per_budget is not None:
      fig, ax = plt.subplots()
    ...
    plt.ylim([0.1, 0.9])
    plt.savefig(
        os.path.join(save_dir, f"budget_vs_noise_vs_acc{additional_label}.png")
    )
    ...
    for noise, results in results_per_noise.items():
        ax.plot(
            results.keys(), results.values(), marker="o", label=f"noise {noise}"
        )
        ax.set_xlabel("Budget")
        ax.set_ylabel("Accuracy")
        ax.set_title("Budget vs Accuracy across noises")
    ...
    plt.ylim([0.1, 0.9])
    plt.savefig(
        os.path.join(save_dir, f"noise_vs_budget_vs_acc{additional_label}.png")
    )
    ...
    if accs_per_flops is not None:
        ...
        for budget, results in accs_per_flops.items():
            ax.plot(
                results.keys(), results.values(), marker="o", label=f"budget {budget}"
            )
            ax.set_xlabel("Noise")
            ax.set_ylabel("Accuracy")
        ...
        # plt.ylim([0.4, 0.9])
        plt.savefig(
            os.path.join(save_dir, f"flops_vs_noise_vs_acc{additional_label}.png")
        )


def plot_cumulative_budget_and_noise_recap(
    run_accs_per_flops, save_dir, additional_x_labels="", run_names=None
):
    ...
    for noise, exps in results_per_noise.items():
        plot_cumulative_budget_recap(
            run_accs_per_budget=None,
            run_accs_per_flops=exps,
            save_dir=save_dir,
            additional_label=f"_noise_{noise}",
            run_names=run_names,
        )


def plot_cumulative_budget_and_noise_recap_old(
    run_accs_per_flops, save_dir, additional_x_labels=""
):
    ...
    for run in run_accs_per_flops:
        ...
        for budget in run_accs_per_flops[run]:
            for noise in run_accs_per_flops[run][budget]:
                _results_per_noise[run][noise][
                    budget if budget not in {0.0, float("inf")} else 1.1
                ] = run_accs_per_flops[run][budget][noise]
    ...
    for run, results_per_noise in _results_per_noise.items():
        ...
        for noise, results in results_per_noise.items():
            ax.plot(
                results.keys(),
                results.values(),
                marker="o" if not is_base_run else "*",
                label=f"noise {noise}",
                color=string_to_color(str(noise)),
            )
            ax.set_xlabel(
                f'Budgets {additional_x_labels if additional_x_labels is not None else ""}'
            )
            ax.set_ylabel("Accuracy")
            ax.set_title("Budget vs Accuracy across Noises")
            # set y range
    ...
    plt.legend(
        by_label.values(), by_label.keys(), bbox_to_anchor=(1.05, 1.0), loc="upper left"
    )
...
def plot_model_noise_vs_budget_vs_acc(
    results_per_model: dict, save_dir: str = None, additional_x_labels: List = None
):
    # results per budget is a dict of dicts, where the first key is the budget and
    # the second key is the noise value, and the value is the accuracy.
    # we want to create a plot where the x axis is the budget, the y axis is the accuracy,
    ...
    for run in results_per_model:
        ...
        for budget in results_per_model[run]:
            for noise in results_per_model[run][budget]:
                _results_per_noise[run][noise][
                    budget if budget not in {0.0, float("inf")} else 1.1
                ] = results_per_model[run][budget][noise]
...
@torch.no_grad()
def img_expert_distribution(
    model, images: List, subset, transform: Optional[None] = None, save_dir: str = None
):
    """
    ...
    )  # it could be channel first or channel last
    ...
    for img_idx in tqdm(subset, desc="Preparing expert distribution plots"):
        ...
        gates = get_last_forward_gates(
            model
        )  # <moe, gating_probs>, each gating_probs is (batch, tokens, exp)
        ...
        for moe_idx, (moe_name, gating_probs) in enumerate(gates.items()):
            ...
            exp_assignment = max_idx[:, num_class_tokens + num_registers :].reshape(
                -1, patches_per_side, patches_per_side
            )  # discard class token and reshape as image
            img, exp_assignment = prepare_for_matplotlib(img), prepare_for_matplotlib(
                exp_assignment
            )
        ...
        if save_dir is not None:
            ...
            plt.savefig(
                join(save_dir, f"expert_distribution_batch_{img_idx}.jpg"), dpi=100
            )
        plt.close()
    ...
    for moe_name, moe in moes.items():
        ...
        fig = go.Figure(
            go.Scatter3d(
                ...
                mode="markers",
                marker=dict(size=12, color=list(range(embs.shape[0])), opacity=0.8),
            )
        )
...
def plot_masked_images(
    model, images, model_transform=None, visualization_transform=None, hard=True
):
    model.eval()
    device = get_model_device(model)
    num_registers = getattr(model, "num_registers", 0)
    ...
    for img, label in tqdm(images, desc="Preparing masked images plots"):
        ...
        fig, axs = plt.subplots(
            len(gates.keys()) + 1, 1, squeeze=False, figsize=(10, 25)
        )
        ...
        for layer_idx, (layer_name, forward_mask) in enumerate(gates.items()):
            ...
            forward_mask = (
                forward_mask[:, num_class_tokens + num_registers - 1 :]
                .detach()
                .reshape(-1, patches_per_side, patches_per_side)
            )  # discard class token and reshape as image
            ...
            cbar = axs[layer_idx + 1, 0].figure.colorbar(
                im, ax=axs[layer_idx + 1, 0], orientation="horizontal", shrink=0.2
            )
...
def plot_class_tokens(model, input, save_dir=None, savepath=None):
    ...
    assert (save_dir is None) != (
        savepath is None
    ), "Either save_dir or savepath must be specified, but not both."
...
def plot_reconstructed_images(
    ...
    for img, label in tqdm(images_to_plot, desc="Preparing reconstructed images plots"):
        ...
        reconstructed = prepare_for_matplotlib(
            visualization_transform(reconstructed * (1 - mask)).squeeze()
        )
