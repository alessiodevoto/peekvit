...
def create_class_token_plots(
    ...
    if model_checkpoint_path is not None:
        model, _, epoch, _, _ = load_state(
            model_checkpoint_path, model=model, strict=True
        )
    ...
    for i, (image, label) in enumerate(images_to_plot):
        ...
        plot_class_tokens_distances(
            model, image, savepath=join(plots_dir, f"cls_tokens_distances_{i}.png")
        )
