...
classfy_images=(True) # True False
...
for trace in ${trace_loss[@]}; do
    for r_val in ${r[@]}; do
        for model in ${model_type[@]}; do
            for snr in ${validate_snr[@]}; do
                for train_snr in ${train_snr_dbs[@]}; do
                    for classfy_image in ${classfy_images[@]}; do
                        python ltrain/train.py decoder.use_trace_loss=$trace encoder.r=$r_val model_type=$model logger.wandb_project=$project_name reconstruct_images=$reconstruct_images validate_snr_db=$snr transmit_cls_token=$transmit_cls_token train_snr_db=$train_snr classfy_images=$classfy_image
                    done
                done
            done
        done
