...
def train(cfg: DictConfig):
    ...
    def train_epoch(model, loader, optimizer, epoch):
        ...
        for batch, labels in tqdm(loader, desc=f"Training epoch {epoch}"):
            ...
            reconstruction_loss = torch.mean(
                ((batch - reconstructed_input) ** 2) * mask
            )  # TODO only compute loss on pixels that are masked
    ...
    def validate(model, loader, epoch):
        ...
        if hasattr(model, "set_budget"):
            for budget in val_budgets:
                ...
                acc, val_loss = validate_epoch(
                    model, loader, epoch, budget=f"budget_{budget}"
                )
                logger.log(
                    {
                        f"budget_{budget}/val/accuracy": acc,
    ...
    def plot_reconstructed_images_in_training(model, budgets):
        ...
        for budget in budgets:
            ...
            os.makedirs(f"{experiment_dir}/images/epoch_{epoch}", exist_ok=True)
            os.makedirs(
                f"{experiment_dir}/images/epoch_{epoch}/reconstructed_budget_{budget}",
                exist_ok=True,
            )
            for i, (_, img) in enumerate(images.items()):
                img.savefig(
                    f"{experiment_dir}/images/epoch_{epoch}/reconstructed_budget_{budget}/reconstructed_img_{subset_idcs[i]}.png"
                )
    ...
    for epoch in range(training_args["num_epochs"] + 1):
        ...
        if (
            training_args["plot_reconstructed_images_every"] != -1
            and epoch % training_args["plot_reconstructed_images_every"] == 0
        ):
