...
for TCS in ${train_classifier_separetely_options[@]}; do
    for train_snr in ${train_snr_dbs[@]}; do
        for trace_loss in ${trace_losses[@]}; do
            for encoding_dim in ${encoding_dims[@]}; do
                for r_val in ${r[@]}; do
                    python ltrain/train_baselines.py compressor=tome_ae compressor.encoding_dim=$encoding_dim train_classifier_separetely=$TCS train_snr_db=$train_snr compressor.use_trace_loss=$trace_loss compressor.r=$r_val logger.wandb_project=$project_name
                done
            done
        done
    done  
