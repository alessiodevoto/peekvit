...
def build_model(model_class, model_args, noise_args=None):
    ...
    if model_class not in MODELS_MAP:
        raise ValueError(
            f"Unknown model class {model_class}. Available models are {list(MODELS_MAP.keys())}"
        )
    ...
    if torch_pretrained_weights:
        state_dict = adapt_torch_state_dict(
            torch_pretrained_weights, model_args["num_classes"]
        )
    ...
    if noise_args is not None and noise_args != {}:
        ...
        print(
            "Loaded model with noise. Noise will be set to 0.0, you can change this by calling model.noise_module.set_value(new_noise_value)"
        )
