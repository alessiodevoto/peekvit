...
class MAEVisionTransformerEncoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        register_tokens = (
            x[:, self.num_class_tokens : self.num_class_tokens + self.num_registers]
            if self.num_registers > 0
            else None
        )
...
class MAEVisionTransformerDecoder(torch.nn.Module):
    def __init__(
        ...
        self.pos_embedding = torch.nn.Parameter(
            torch.empty(1, seq_length - 1, decoder_hidden_dim).normal_(std=0.02)
        )
        self.image_size = image_size
        self.patch_size = patch_size
    ...
    def forward(self, tokens, backward_indices=None, mask=None):
        ...
        assert (
            backward_indices or mask
        ), "Either backward_indices or mask must be provided"
