...
class EEResidualViTEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.early_exit_heads = nn.ModuleList(
            [
                nn.Sequential(
                    nn.LayerNorm(hidden_dim), nn.Linear(hidden_dim, self.num_classes)
                )
    ...
    def forward(self, input: torch.Tensor):
        ...
        for i in range(self.num_layers):
            ...
            early_exits.append(
                self.early_exit_heads[i](input[:, 0 : self.num_class_tokens]).squeeze()
            )
            ...
            if isinstance(self.budget, float):
                ...
                x = torch.cat(
                    [
                        ...
                        + (1 - self.current_budget) * batch_budget_token_2,
                    ],
                    dim=1,
                )
                return x
