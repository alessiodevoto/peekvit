...
def add_register_tokens(model, num_registers):
    ...
    model.register_tokens = torch.nn.Parameter(
        torch.zeros(1, num_registers, model.hidden_dim)
    )
...
def add_residual_gates(residualvit_model, residual_gates_args):
    ...
    for module_name, module in residualvit_model.named_modules():
        if isinstance(module, ResidualViTBlock) and skip[i] in {
            "attention+mlp",
            "attention",
            "mlp",
        }:
            ...
            module.residual_gate = ResidualGate(
                module.hidden_dim, temp=temp, gate_type=gate_type
            )
            i += 1
...
def train_only_these_params(model, params_list: List, verbose: bool = False):
    ...
    print(
        "Freezing all parameters except for those containing any of these words in their names: ",
        params_list,
    )
