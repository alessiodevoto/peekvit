...
class RankingPCTBlock(nn.Module):
    ...
    def sort_tokens(input: torch.Tensor):
        ...
        # gather sorted input
        sorted_input = torch.gather(
            input, dim=1, index=sorted_indices.expand(-1, -1, input.shape[-1])
        )
...
class ARPE(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat(
            [x.unsqueeze(2).repeat(1, 1, self.k, 1), diffs], dim=-1
        )  # B, N, K, 2*C
        x = F.elu(
            self.bn1(self.lin1(x.view(B * N, self.k, 2 * C)).transpose(1, 2)).transpose(
                1, 2
            )
        )  # B*N, K, 2*C
        x = self.max_pooling_layer(x).squeeze(2)  # B*N, 1, 2*C -> B*N, 2*C
        x = F.elu(
            self.bn2(self.lin2(x.view(B, N, 2 * C)).transpose(1, 2)).transpose(1, 2)
        )  # B, N, out_channels
...
class RankPointCloudTransformer(nn.Module):
    ...
    def __init__(
        ...
        if torch_pretrained_weights is not None:
            ...
            adapted_state_dict = adapt_torch_state_dict(
                torch_pretrained_weights, num_classes=num_classes
            )
            self.load_state_dict(adapted_state_dict, strict=False)

    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        torch._assert(
            x.dim() == 3, f"Expected (batch_size, num_points, channels) got {x.shape}"
        )
