...
class NoiseBlock(nn.Module):
    def __init__(
        self,
        noise_type: Literal["gaussian", "token_drop"] = "gaussian",
        snr=None,
        std=None,
        prob=None,
    ):
        ...
        if not any([snr, std, prob]):
            print(
                "Lazy initialization of noise block. Please set the noise parameters using set_snr, set_std or set_prob before using the block."
            )
    ...
    def forward_snr(self, x: torch.Tensor):
        ...
        noise_power = (
            (signal_power / (10 ** (self.snr_db / 10))) if self.snr_db != 0 else 0
        )
