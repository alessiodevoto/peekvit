...
class ResidualGate(nn.Module):
    ...
    def forward(self, x, budget: float = None, threshold: float = None):
        ...
        assert (
            budget is None or threshold is None
        ), "Cannot specify both budget and threshold"
...
class ResidualViTBlock(ResidualModule):
    ...
    def forward_skip_attention(self, input: torch.Tensor):
        ...
        self.mask = self.residual_gate(
            img_tokens, budget=budget_token.mean() if self.budget_token else None
        )
    ...
    def forward_skip_mlp(self, input: torch.Tensor):
        ...
        if self.add_input:
            ...
            y = y + torch.cat(
                [torch.zeros_like(special_tokens), unmasked_tokens], dim=1
            )
    ...
    def forward_skip_attention_mlp(self, input: torch.Tensor):
        ...
        self.mask = self.residual_gate(
            img_tokens, budget=current_budget, threshold=threshold
        )
...
class ResidualVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        gate_bias: float = 10.0,
        gate_threshold: float = 0.5,
        add_budget_token: Union[
            bool, List, Literal["learnable", "learnable_interpolate"]
        ] = False,
        ...
        if self.add_budget_token:
            ...
            if self.add_budget_token == "learnable":
                self.learnable_budget_token_1 = nn.Parameter(
            ...
            if self.add_budget_token == "learnable_interpolate":
                ...
                self.learnable_budget_token_2 = nn.Parameter(
                    torch.randn(1, 1, hidden_dim)
                )
                seq_length += 1
    ...
    def _sample_budget(self):
        ...
        if isinstance(self.add_budget_token, (list, tuple)):
            ...
            return (
                torch.rand(1) * (self.budget_interval[1] - self.budget_interval[0])
                + self.budget_interval[0]
            )
    ...
    def _add_budget_token(self, x):
        ...
        if self.training:
            ...
            assert (
                self.current_budget is not None
            ), "Budget token not set. Call set_budget() before forward() to evaluate the model on a chosen budget."

        if self.add_budget_token == "learnable":
            batch_budget_token_1 = (
                self.learnable_budget_token_1.expand(n, -1, -1) * self.current_budget
            )
            ...
            batch_budget_token_2 = self.learnable_budget_token_2.expand(n, -1, -1) * (
                1 - self.current_budget
            )
            x = torch.cat([x, batch_budget_token_1 + batch_budget_token_2], dim=1)
        else:
            budget_token = torch.empty((n, 1, self.hidden_dim), device=x.device).fill_(
                self.current_budget
            )
